...
async function dryRunSanitizeEntities() {
  ...
  try {
      ...
      if (decoded !== genre.name) {
        ...
        if (existing) {
          ...
          if (albumsWithEncodedGenre.length > 0) {
            console.log(
              `  - Affected albums: ${albumsWithEncodedGenre
                .slice(0, 3)
                .map((a) => a.title)
                .join(', ')}${albumsWithEncodedGenre.length > 3 ? '...' : ''}`
            )
          }
        } else {
          // WOULD RENAME
    ...
    for (const descriptor of descriptors) {
      ...
      if (decoded !== descriptor.name) {
        ...
        if (existing) {
          ...
          console.log(
            `  - Would transfer ${albumsWithEncodedDescriptor.length} album relationships`
          )
          console.log(`  - Would delete duplicate descriptor (ID: ${descriptor.id})`)
          if (albumsWithEncodedDescriptor.length > 0) {
            console.log(
              `  - Affected albums: ${albumsWithEncodedDescriptor
                .slice(0, 3)
                .map((a) => a.title)
                .join(', ')}${albumsWithEncodedDescriptor.length > 3 ? '...' : ''}`
            )
          }
        } else {
          // WOULD RENAME
    ...
    console.log(
      `Found ${descriptorRenames} descriptors to rename, ${descriptorConsolidations} to consolidate\n`
    )
      ...
      if (decoded !== artist.name) {
        ...
        if (existing) {
          ...
          if (albumsWithEncodedArtist.length > 0) {
            console.log(
              `  - Affected albums: ${albumsWithEncodedArtist
                .slice(0, 3)
                .map((a) => a.title)
                .join(', ')}${albumsWithEncodedArtist.length > 3 ? '...' : ''}`
            )
          }
        } else {
          // WOULD RENAME
    ...
    console.log(
      `Found ${artistRenames} artists to rename, ${artistConsolidations} to consolidate\n`
    )
